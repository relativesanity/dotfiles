...
-- which can raise whichever window was last active before settling on the
-- right one (visible as a flicker in Ghostty). Five seconds covers a cold
-- launch; if the window still isn't there by then, something's wrong and
-- keeping the app pinned open no longer helps.
--
-- Polling instead of an event lets the wait get interrupted cleanly: if
-- another app activates while we're waiting, that's focus moving
-- deliberately elsewhere, so the pending focus is dropped instead of
-- yanking it back once the window turns up. Window ids only increase, so
-- the highest new id is the one this call actually produced.
--
-- hs.window.filter's windowCreated event would replace this poll with a
-- proper accessibility callback instead of timer bookkeeping. Not worth
-- the swap while this stays a small helper with one caller.
