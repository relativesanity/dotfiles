...
- Homebrew's tap-trust file (`~/.homebrew/trust.json`) is NOT tracked or stowed — brew owns it as a real local file. `repack`/`redot` reassert it before bundling by running `brew trust <tap>` for every `tap` line in the active Brewfiles, so it stays correct per environment (home and work declare different taps) and survives brew's own rewrites. Don't stow it: brew deletes the trust file whenever its store goes empty (`write_trust_store` → `unlink`), and because it resolves through a symlink that would delete the tracked repo file. To trust a new tap, just add its `tap` line to the right Brewfile and re-run `dot pack`.
