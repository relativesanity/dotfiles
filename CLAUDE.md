...
- `Brewfile.cache` is auto-generated by `repack` (gitignored, loaded last): before bundling it records mas apps and casks that are installed but not declared in any tracked Brewfile, so `brew bundle --zap` won't uninstall them. It's recomputed every run against the tracked Brewfiles only — promote keepers into `Brewfile.home`/`Brewfile.work` and they drop out of the cache
