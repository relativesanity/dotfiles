...
- The Meh/Hyper keybinding scheme spanning AeroSpace, Hammerspoon, tmux, Neovim, and (via the separate `dotfiles-omarchy` repo) Hyprland has its design rationale in `Keybindings.md`, not inlined in each config — check it before changing any of those bindings
...
- `aerospace`/`borders`/`sketchybar` (the tiling-WM set) are declared in `Brewfile.home`, so every home machine installs and runs them — this was a deliberate promotion from `Brewfile.local`, once occasional use became the norm; see `REQUIRED_DIRECTORIES` in `restow.sh` for their stowed config dirs
- `reenv` is a thin wrapper around `mise install`, driven by one file: `mise/.config/mise/config.toml` (pinned Ruby + Node versions, plus language-toolchain and editor tooling via mise's `gem:`/`npm:` backends and core plugins). The dividing line from the Brewfiles is not "does Homebrew have a formula" — it's whether the tool is a dev-toolchain or editor component (a language runtime, LSP server, linter, formatter, or something else LazyVim's extras depend on) versus a general system/app utility. `ruby-lsp`, `rubocop`, `tailwindcss-language-server`, `@olrtg/emmet-language-server`, and `tree-sitter` all live here for that reason, even though several of them also have a Homebrew formula. `reenv`'s plan step is `mise install --dry-run`; mise decides what's already installed, so there's no bespoke diffing logic to maintain
